...
function xmldb_scorm_upgrade($oldversion) {
    ...
    if ($oldversion < 2013112200) {
        ...
        $field = new xmldb_field('compatphone', XMLDB_TYPE_INTEGER, '1', XMLDB_UNSIGNED, true, null, 0, 'completionscorerequired');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('compattablet', XMLDB_TYPE_INTEGER, '1', XMLDB_UNSIGNED, true, null, 0, 'compatphone');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
